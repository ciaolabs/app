...
import {
  ...
  loadInternalQuestionDistributionsSafe,
} from "@/lib/survey/results/reference-distributions";

export default async function PersonalitySurveyPage() {
  const [initialDraft, distributionSet] = await Promise.all([
    getRequiredSurveyDraft(personalitySurveyDefinition.type),
    loadInternalQuestionDistributionsSafe(personalitySurveyDefinition.type),
  ]);
  const questions = applyReferenceDistributions(
    personalitySurveyDefinition.questions,
    distributionSet,
  );
