...
describe("auth helpers", () => {
  ...
  it("prefers request-bound browser auth in route handlers", async () => {
    ...
    const { getCurrentUserId } = await import("@/lib/auth");
    const request = new Request("http://localhost/api/survey/submit", { method: "POST" });
  ...
  it("verifies the bearer token directly when Clerk request auth returns signed out", async () => {
    ...
    getClerkSecretKeyMock.mockReturnValue("sk_test_123");

    const { getCurrentUserId } = await import("@/lib/auth");
    const request = new Request("http://localhost/api/survey/answer", {
      method: "PUT",
      headers: {
