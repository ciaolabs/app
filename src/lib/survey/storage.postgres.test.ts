  ...
  const sql = (async (strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    if (query.includes("from survey_submissions s") && query.includes("s.status = 'draft'")) {
      ...
      const draft = database.submissions.find(
        (submission) => submission.user_id === userId && submission.status === "draft",
      );
    ...
    if (
      ...
      const submission = database.submissions
        .filter((row) => row.user_id === userId && row.status === "submitted" && row.submitted_at)
    ...
    if (
      ...
      const [userId, submissionId] = values as [string, string];
    ...
    if (
      query.startsWith("select s.id, s.user_id, s.answer_count, s.created_at, s.updated_at, s.submitted_at") &&
...
describe("createPostgresSurveyRepository", () => {
  ...
  it("stores a submitted survey in the normalized postgres schema", async () => {
    ...
    const submission = await repository.submitDraft({ userId, answers });
  ...
  it("keeps submission history and resolves the newest submitted survey", async () => {
    ...
    const firstSubmission = await repository.submitDraft({ userId, answers: firstAnswers });
    const freshDraft = await repository.ensureDraft(userId);
    const secondSubmission = await repository.submitDraft({ userId, answers: secondAnswers });
    const latestSubmission = await repository.getLatestSubmission(userId);
