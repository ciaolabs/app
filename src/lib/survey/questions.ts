...
import { valuesBeliefsSurveyQuestions } from "@/lib/survey/questions/values-beliefs";

const PERSONALITY_RESPONSE_SCALE: QuestionResponseScale = {
  id: "personality-accuracy",
  leftAnchor: "Inaccurate",
  rightAnchor: "Accurate",
  options: LIKERT_LABELS.map((label, index) => ({
...
export const personalitySurveyQuestions: QuestionItem[] = rawQuestionItems.map(
  ([id, stem], index) => {
    ...
    return {
      ...
      order: index + 1,
      prompt,
      responseScale: PERSONALITY_RESPONSE_SCALE,
      visual: {
        kind: "violin",
        distribution: seededDistributionFor(id, prompt, index + 1),
...
if (rawQuestionItems.length !== QUESTION_COUNT) {
  throw new Error(
    `Survey question count is out of sync. Expected ${QUESTION_COUNT}, received ${rawQuestionItems.length}.`,
  );
}

export const surveyQuestions = personalitySurveyQuestions;

export const QUESTION_IDS = new Set(personalitySurveyQuestions.map((question) => question.id));
export const QUESTION_LOOKUP = new Map(
  personalitySurveyQuestions.map((question) => [question.id, question]),
);

const surveyQuestionMap: Record<SurveyType, readonly QuestionItem[]> = {
  personality: personalitySurveyQuestions,
  "values-beliefs": valuesBeliefsSurveyQuestions,
};

export function getSurveyQuestions(surveyType: SurveyType) {
  return surveyQuestionMap[surveyType];
}
